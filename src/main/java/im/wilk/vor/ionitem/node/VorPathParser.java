...
public class VorPathParser {
    ...
    public VorPathParser(PathParserConfig config) {
    ...
    public List<VorPathNode> toNodes(String path) {

        List<VorPathNode> nodes = new ArrayList<>();
    ...
    public String buildPath(Collection<VorPathNode> forNodes) {
        StringBuilder sb = new StringBuilder();
        for (VorPathNode forNode : forNodes) {
    ...
    private void addNode(List<VorPathNode> nodes, StringBuilder name, StringBuilder alias, StringBuilder index) {
        if (name.length() > 0) {
            nodes.add(VorPathNode.forName(name.toString(), alias.toString()));
            name.setLength(0);
        }
        if (index.length() > 0) {
            int indexValue = Integer.parseInt(index.toString());
            nodes.add(VorPathNode.forIndex(indexValue, alias.toString()));
            index.setLength(0);
        }
        alias.setLength(0);
    }
